...
def get_importance():
    ...
    regressor = RandomForestRegressorModel(n_estimators=100, random_state=0)
    regressor.fit(X_tr, y_tr)

    importance = regressor.get_feature_importances()
    xs = arange(len(importance))  # Change `ys` to `xs` for clarity
    plot = figure(figsize=(8, 6))  # Optionally adjust the figure size for better visuals
    ax = plot.add_subplot()
    ...
    ax.set_xticks(xs)
    ax.set_xticklabels(list(X), rotation=45, ha='right', fontsize=10)  # Rotate labels for clarity
